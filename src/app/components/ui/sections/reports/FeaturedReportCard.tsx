...
export default function FeaturedReportCard({
  ...
  return (
    <Link
      ...
      className="block border border-[#0078FF]/20 bg-[#0078FF]/[0.04]
        p-[28px] [@media(max-width:640px)]:p-[20px]
        hover:border-[#0078FF]/40 hover:shadow-[0_4px_24px_rgba(0,120,255,0.12)]
    ...
    >
      ...
      <h3 className="text-[22px] [@media(max-width:640px)]:text-[18px] font-[600] text-white group-hover:text-[#0078FF] transition-colors duration-200 mb-[4px]">
        Biweekly{report.reportNumber != null && (
          <span className="text-[#0078FF] font-[700]"> #{report.reportNumber}</span>
        )}
      </h3>
      <p className="text-[15px] text-[#929298] mb-[8px]">{report.dateLabel}</p>

      {report.executiveHeadline && (
        <p className="text-[15px] text-[#c5c5ca] leading-[1.5] mb-[20px] line-clamp-2">
          {report.executiveHeadline}
        </p>
      )}

      <div className="bg-surface p-[16px] [@media(max-width:640px)]:p-[12px] mb-[20px]">
